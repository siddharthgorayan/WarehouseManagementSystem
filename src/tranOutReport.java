...
public class tranOutReport extends javax.swing.JFrame {

    Connection con;
    Statement stmt;
    ResultSet result;
    public tranOutReport() {
        initComponents();
        Connect();
        loadData();
    }
      ...
      public void loadData()
    {
        try
        {
            String data = "SELECT * FROM transactionOUT";
            result  = stmt.executeQuery(data);
            DefaultTableModel model = (DefaultTableModel)tableOUT.getModel();
            model.setRowCount(0);
            while(result.next())
            {
                model.addRow(new Object[]{result.getString("srno"),result.getString("productID"),result.getString("productName"),result.getString("cost"),result.getString("units"),result.getString("totalCost"),result.getString("customerID"),result.getString("customerName"),result.getString("Date"),result.getString("Time")});
            }
        }
        catch(Exception ex)
        {
    ...
    private void initComponents() {
        ...
        tableOUT.setFont(new java.awt.Font("Futura", 0, 14)); // NOI18N
        tableOUT.setForeground(new java.awt.Color(0, 102, 0));
        tableOUT.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Transaction ID", "Product ID", "Product Name", "Cost", "Units ", "Total Cost", "Customer ID", "Customer Name", "Date", "Time"
            }
        ) {
    ...
    public static void main(String args[]) {
        ...
        try {
            ...
            java.util.logging.Logger.getLogger(tranOutReport.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
        ...
        java.awt.EventQueue.invokeLater(new Runnable() {
            public void run() {
                new tranOutReport().setVisible(true);
            }
        });
    }

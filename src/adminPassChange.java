...
public class adminPassChange extends javax.swing.JFrame {
    ...
    private void doneTFActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_doneTFActionPerformed
        ...
        try
        {
            String data = "SELECT * FROM adminLogin WHERE adminName = '"+id+"'";
            result = stmt.executeQuery(data);
            
            if(result.next())
            {
                if(oPass.equals(result.getString("adminPassword")))
                {
                    if(nPass.equals(cPass))
                    {
                        try
                        {
                            String data1 = "UPDATE adminLogin SET adminPassword = '"+nPass+"' WHERE adminName = '"+id+"'";
            ...
            {
                JOptionPane.showMessageDialog(this, "Admin not found! Try Again.");
            }
        }
        catch(Exception ex)
        {
